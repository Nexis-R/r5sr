...
void DetectionPanel::onInitialize() {
  ...
  barcode_sub = node->create_subscription<std_msgs::msg::String>(
      "barcode", 1,
      std::bind(&DetectionPanel::handle_barcode, this, std::placeholders::_1));
}

void DetectionPanel::handle_barcode(
    const std_msgs::msg::String::SharedPtr msg) {
  barcode_label->setText(QString::fromStdString(msg->data));
}

void DetectionPanel::save(rviz_common::Config config) const {
  rviz_common::Panel::save(config);
}

void DetectionPanel::load(const rviz_common::Config &config) {
