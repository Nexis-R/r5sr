...
public:
  ...
  : rviz_common::Panel(parent), start_time_(std::chrono::steady_clock::now())
  {
...
private Q_SLOTS:
  void resetTimer() { start_time_ = std::chrono::steady_clock::now(); }
  ...
  void updateTime()
  {
    ...
    time_label_->setText(
      "経過時間: " + QString::number(minutes) + "分 " + QString::number(seconds) + "秒");
  }
