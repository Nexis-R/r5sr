...
ModePanel::ModePanel(QWidget * parent) : rviz_common::Panel(parent)
{
  ...
  connect(timer, &QTimer::timeout, this, [&]() { is_stopped_pub->publish(is_stopped); });
  connect(emergencyButton, &QPushButton::toggled, this, [&](bool is_checked) {
    is_stopped.set__data(is_checked);
  });
  connect(emergencyButton, &QPushButton::toggled, emergencyButton, &QPushButton::setDisabled);
}
...
void ModePanel::onInitialize()
{
  ...
  is_stopped_pub = node->create_publisher<std_msgs::msg::Bool>("is_emergency_stopped", 1);

  timer->start(500);
}

void ModePanel::save(rviz_common::Config config) const { rviz_common::Panel::save(config); }

void ModePanel::load(const rviz_common::Config & config) { rviz_common::Panel::load(config); }
