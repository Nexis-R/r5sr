...
{
public:
  PresetPose(const rclcpp::NodeOptions & options) : Node("preset_pose", options)
  {
    joint_trajectory_pub = this->create_publisher<trajectory_msgs::msg::JointTrajectory>(
      "/arm_controller/joint_trajectory", 10);
    stop_servo_client = this->create_client<std_srvs::srv::Trigger>("/servo_node/stop_servo");
    start_servo_client = this->create_client<std_srvs::srv::Trigger>("/servo_node/start_servo");
    ...
    move_to_default_pose_service = this->create_service<std_srvs::srv::Empty>(
      "~/move_to_default_pose", std::bind(
                                  &PresetPose::moveToDefaultPoseCallback, this,
                                  std::placeholders::_1, std::placeholders::_2));
    move_to_floor_pose_service = this->create_service<std_srvs::srv::Empty>(
      "~/move_to_floor_pose",
      std::bind(
        &PresetPose::moveToFloorPoseCallback, this, std::placeholders::_1, std::placeholders::_2));
    ...
    move_to_dex1_center_pose_service = this->create_service<std_srvs::srv::Empty>(
      ...
      std::bind(
        &PresetPose::moveToDex1CenterPoseCallback, this, std::placeholders::_1,
        std::placeholders::_2));
    move_to_dex1_diaright_pose_service = this->create_service<std_srvs::srv::Empty>(
      "preset_pose_node/move_to_dex1_diaright_pose",
      std::bind(
        &PresetPose::moveToDex1DiaRightPoseCallback, this, std::placeholders::_1,
        std::placeholders::_2));
    move_to_dex1_dialeft_pose_service = this->create_service<std_srvs::srv::Empty>(
      "preset_pose_node/move_to_dex1_dialeft_pose",
      std::bind(
        &PresetPose::moveToDex1DiaLeftPoseCallback, this, std::placeholders::_1,
        std::placeholders::_2));
    move_to_dex1_right_pose_service = this->create_service<std_srvs::srv::Empty>(
      "preset_pose_node/move_to_dex1_right_pose", std::bind(
                                                    &PresetPose::moveToDex1RightPoseCallback, this,
                                                    std::placeholders::_1, std::placeholders::_2));
    move_to_dex1_left_pose_service = this->create_service<std_srvs::srv::Empty>(
      "preset_pose_node/move_to_dex1_left_pose", std::bind(
                                                   &PresetPose::moveToDex1LeftPoseCallback, this,
                                                   std::placeholders::_1, std::placeholders::_2));
  }
...
private:
  ...
  {
    // stop servo
    auto stop_servo_future =
      stop_servo_client->async_send_request(std::make_shared<std_srvs::srv::Trigger::Request>());
    ...
    joint_trajectory.joint_names = {"body_joint",    "shoulder_joint",  "elbow_joint",
                                    "forearm_joint", "wrist_yaw_joint", "wrist_roll_joint"};
    ...
    // start servo
    auto start_servo_future =
      start_servo_client->async_send_request(std::make_shared<std_srvs::srv::Trigger::Request>());
  ...
  {
    moveToPositions(
      {-0.005899328800385288, 0.7083160049999053, -0.6195460128516843, 1.4261574842731133,
  ...
  {
    moveToPositions(
      {-2.972387139899503e-05, 0.38043264539814864, -0.6489122199681415, 0.27611256317117366,
  ...
  {
    moveToPositions(
      {0.01991229029153637, 1.6294818377236404, -1.0059121381250555, 0.8913840174821814,
    ...
    const std_srvs::srv::Empty::Response::SharedPtr response)
  {
    ...
    moveToPositions(
      {0.014407121838988318, 1.5085494110096298, -1.0110233745048391, 1.0174188269794784,
       -0.0015236838048826179, 0.014337062917931768});

    moveToPositions(
      {-0.5557149518823368, 1.896163636947308, -1.1273298882506417, 1.0606426782545288,
    ...
    const std_srvs::srv::Empty::Response::SharedPtr response)
  {
    ...
    moveToPositions(
      {0.5341102027497134, 1.9730644846282765, -1.191104160197627, 1.151873752530019,
    ...
    const std_srvs::srv::Empty::Response::SharedPtr response)
  {
    ...
    moveToPositions(
      {-0.45375902007816393, 1.7886923464987132, -1.168202155866634, 0.899908021980262,
    ...
    const std_srvs::srv::Empty::Response::SharedPtr response)
  {
    ...
    moveToPositions(
      {0.5059943281768762, 1.804458871384405, -1.1532734772612905, 0.8711237825894158,
