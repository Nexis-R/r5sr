...
Joy2Command::Joy2Command(const rclcpp::NodeOptions& options) : Node("joy2command", options)
{
  ...
  twist_arm_publisher = this->create_publisher<geometry_msgs::msg::TwistStamped>("delta_twist_cmds", 10);
  joint_arm_publisher = this->create_publisher<control_msgs::msg::JointJog>("joint_jog", 10);
  joint_overhead_arm_publisher = this->create_publisher<control_msgs::msg::JointJog>("joint_jog_overhead", 10);
  ...
  mode_change_publisher = this->create_publisher<r5sr_interface::msg::OperationMode>("mode_change", 10);
  ...
  overhead_servo_start_client = this->create_client<std_srvs::srv::Trigger>("/overhead_servo_node/start_servo");
  overhead_servo_start_client->wait_for_service(std::chrono::seconds(1));
  overhead_servo_start_client->async_send_request(std::make_shared<std_srvs::srv::Trigger::Request>());
  ...
  switch (operation_mode.teleop_mode)
  {
    case r5sr_interface::msg::TeleopMode::CRAWLER:
      ...
      // crawler
      {
        ...
        twist_msg.linear.x =
            (operation_mode.crawler_mode == r5sr_interface::msg::CrawlerMode::FORWARD) ? linear_x : -linear_x;
      ...
      // flippers
      {
        auto flipper_speed = [](const bool button, const float stick) -> float { return button ? stick * 0.005 : 0.0; };

        this->flippers_position = {
          flippers_position[0] - flipper_speed(buttons[Button::LEFT_BUMPER], axes[Axis::RIGHT_STICK_Y]),
          flippers_position[1] + flipper_speed(buttons[Button::LEFT_TRIGGER_BUTTON], axes[Axis::RIGHT_STICK_Y]),
          flippers_position[2] + flipper_speed(buttons[Button::RIGHT_TRIGGER_BUTTON], axes[Axis::RIGHT_STICK_Y]),
          flippers_position[3] - flipper_speed(buttons[Button::RIGHT_BUMPER], axes[Axis::RIGHT_STICK_Y]),
        };

        trajectory_msgs::msg::JointTrajectory flippers_msg;
        flippers_msg.joint_names = { "flipper_left_front_joint", "flipper_left_rear_joint", "flipper_right_rear_joint",
